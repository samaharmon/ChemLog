...
async function submitForm(event) {
    ...
    const saved = await saveFormSubmissions(submission);
    ...
    if (saved) {
        ...
        // Show feedback modal
        evaluateFormFeedback(submission);
    }
}
...
async function loadAndDisplayData() {
    try {
        ...
        tbody1.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 30px;">Loading data...</td></tr>';
        tbody2.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 30px;">Loading data...</td></tr>';
        
        // Load data from Firebase
        formSubmissions = await loadFormSubmissions();
        ...
        tbody1.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 30px; color: red;">Failed to load data</td></tr>';
        tbody2.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 30px; color: red;">Failed to load data</td></tr>';
    }
}
...
async function saveFormSubmissions(submissions) {
    try {
        if (Array.isArray(submissions)) {
            // If saving multiple submissions
            for (const submission of submissions) {
                await addDoc(collection(db, 'poolSubmissions'), submission);
            }
        } else {
            // If saving a single submission
            await addDoc(collection(db, 'poolSubmissions'), submissions);
        }
