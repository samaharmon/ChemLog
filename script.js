...
async function initializeSanitationSettings() {
    ...
    // Wait for Firebase to be available (give it a moment to initialize)
    let attempts = 0;
...
async function showSettings() {
    ...
    // Refresh settings from Firebase before showing modal
    try {
        if (window.firebase && window.firebase.db) {
            const { db, doc, getDoc } = window.firebase;
            const settingsDoc = await getDoc(doc(db, 'settings', 'sanitationMethods'));
        ...
        console.warn('Could not refresh from Firebase:', error);
    }
...
async function handleSanitationChange(checkbox) {
    ...
    try {
        ...
        localStorage.setItem('sanitationSettings', JSON.stringify(sanitationSettings));
        console.log('Saved sanitation settings to localStorage as fallback');
    }
}
